...
class TestDiscoveryIntegration:
    ...
    def mock_config(self):
        ...
        sample_config = {
            "server": {"name": "Test Server", "version": "0.2.0"},
            "bigquery": {"billing_project": "test-billing", "service_account_path": ""},
            "projects": [
                {
    ...
    def test_full_discovery_workflow(self, mock_config, mock_bigquery_integration):
        ...
        with patch("config.get_config", return_value=mock_config):
            with patch("client.BigQueryClient") as mock_bq_class:
                ...
                mock_bq_instance.list_datasets.side_effect = (
                    lambda p: mock_bigquery_integration.list_datasets(p)
                )
                mock_bq_instance.list_tables.side_effect = (
                    lambda d, t=None: mock_bigquery_integration.list_tables(
                        mock_bigquery_integration.dataset(
                            d.split(".")[1],
                            project=d.split(".")[0] if "." in d else "test-billing",
                        )
                    )
                )
                mock_bq_instance.parse_dataset_path.side_effect = lambda d: (
                    ("analytics-test", d) if "." not in d else d.split(".", 1)
                )
